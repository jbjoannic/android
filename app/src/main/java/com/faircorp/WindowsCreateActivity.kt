        ...
        lifecycleScope.launch(context = Dispatchers.IO) { // (1)
            ...
            withContext(context = Dispatchers.Main) {
                val arrayAdapterSpinner =
                    ArrayAdapter(applicationContext, R.layout.activity_windows_create_item, names)
                val dataStatus = arrayListOf<String>("OPENED", "CLOSED")
                val arrayAdapterSpinnerStatus = ArrayAdapter(
                    applicationContext,
                    R.layout.activity_windows_create_item,
                    dataStatus
                )
                binding.spinnerWindowsRooms.adapter = arrayAdapterSpinner
                binding.spinnerWindowsStatus.adapter = arrayAdapterSpinnerStatus

                binding.spinnerWindowsRooms.onItemSelectedListener =
                ...
                binding.spinnerWindowsStatus.onItemSelectedListener =
                    object : AdapterView.OnItemSelectedListener {
                        override fun onItemSelected(
                            parent: AdapterView<*>?,
                            view: View?,
