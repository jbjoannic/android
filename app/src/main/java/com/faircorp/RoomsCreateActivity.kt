...
class RoomsCreateActivity : BasicActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch(context = Dispatchers.IO) { // (1)
            ...
            withContext(context = Dispatchers.Main) {
                val arrayAdapterSpinner =
                    ArrayAdapter(applicationContext, R.layout.activity_rooms_create_item, names)
                binding.spinnerRoomsBuildings.adapter = arrayAdapterSpinner

                binding.spinnerRoomsBuildings.onItemSelectedListener =
    ...
    fun createRoom(view: View) {
        ...
        val roomCurrentTemp =
            findViewById<EditText>(R.id.edit_room_current_temp).text.toString().toDouble()
        val roomTargetTemp =
            findViewById<EditText>(R.id.edit_room_target_temp).text.toString().toDouble()
